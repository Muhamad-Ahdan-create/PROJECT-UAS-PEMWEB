...
@section('content')
    ...
    <h5 class="fw-bold mb-4 text-dark">Ringkasan Koperasi</h5>
    ...
    <div class="row g-4">
        ...
        {{-- Grafik Donat Status Pinjaman --}}
        <div class="col-lg-4">
            <div class="card h-100">
                <div class="card-header">
                    <i class="bi bi-pie-chart me-2 text-primary"></i>Status Pinjaman
                </div>
                <div class="card-body d-flex align-items-center justify-content-center">
                    <canvas id="chartPinjaman" style="max-height:220px;"></canvas>
                </div>
            </div>
        </div>
                        ...
                        <tbody>
                            @forelse($pengajuanTerbaru as $p)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge badge-status bg-warning text-dark">{{ ucfirst($p->status) }}</span>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center text-muted py-3 small">Tidak ada pengajuan pending.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                            ...
                            @forelse($pinjamanTerbaru as $p)
                                <tr>
                                    ...
                                    <td>
                                        @php
                                            $cls = match ($p->status) {
                                                'disetujui' => 'bg-success', 'lunas' => 'bg-primary',
                                                'pending' => 'bg-warning text-dark', default => 'bg-secondary',
                                            };
                                        @endphp
                                        <span class="badge badge-status {{ $cls }}">{{ ucfirst($p->status) }}</span>
                                    </td>
                                </tr>
                            @empty
                                <tr>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            new Chart(document.getElementById('chartPinjaman'), {
                type: 'doughnut',
                data: {
                    labels: ['Aktif', 'Lunas', 'Pending', 'Ditolak'],
                    datasets: [{
                        data: [
              {{ \App\Models\Pinjaman::where('status', 'disetujui')->count() }},
              {{ \App\Models\Pinjaman::where('status', 'lunas')->count() }},
              {{ \App\Models\Pinjaman::where('status', 'pending')->count() }},
              {{ \App\Models\Pinjaman::where('status', 'ditolak')->count() }},
                        ],
                        backgroundColor: ['#1a56db', '#16a34a', '#d97706', '#dc2626'],
                        borderWidth: 2,
                        borderColor: '#fff',
                    }]
                },
